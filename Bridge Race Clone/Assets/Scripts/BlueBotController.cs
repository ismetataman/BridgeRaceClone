...
public class BlueBotController : MonoBehaviour
{
    ...
    public void FindCollectables()
    {
        ...
        if(objectsInArea.Length >0)
        {
            ...
            if(closest)
            {
                ...
                {
                    GameObject.Find("BlueStack").transform.GetChild(GameObject.Find("BlueStack").transform.childCount -1).gameObject.GetComponent<BoxCollider>().enabled = false;
                }
                if(StackList.instance.blueStack.Count > 2)
                {
            ...
            if(furthest)
            {
                ...
                if(StackList.instance.blueStack.Count > 8)
                {
                    agent.SetDestination(finalDestination[index].transform.position);
                }
            }
        ...
        else if(objectsInArea.Length == 0 && StackList.instance.blueStack.Count == 0)
        {
